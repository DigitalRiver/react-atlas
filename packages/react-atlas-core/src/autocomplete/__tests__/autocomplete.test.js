...
var badOnChangeProps = {
  value: 'value'
};

describe('Test Autocomplete component', () => {
  it('Test default props', function() {
    const result = mount(<Autocomplete inputProps={inputProps}></Autocomplete>);
    expect(typeof result.props().onSuggestionsUpdateRequested).to.equal('function');
    ...
    expect(result.props().getSectionSuggestions()).to.throw(new Error('`getSectionSuggestions` must be provided'));
  });
  ...
  it('Make sure onChange function can be called', function() {
    const result = shallow(<Autocomplete inputProps={inputProps}></Autocomplete>);
