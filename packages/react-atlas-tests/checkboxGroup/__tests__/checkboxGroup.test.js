...
describe("Test CheckboxGroup component - the bascis", () => {
	it("Checkbox group - 4 checkboxes, 1 checked", function(){
		const component = mount(<CheckboxGroupCore title={'CheckboxGroup test'}>
									<CheckboxCore name={'1'}/>
		                            <CheckboxCore name={'2'}/>
									<CheckboxCore name={'3'}/>
									<CheckboxCore name={'4'}/>
...
function _verifyMinMax(mi, ma, numberOfClicks, message, expectedResult)	{
	const component = mount(<CheckboxGroupCore title={'CheckboxGroup test'} min={mi} max={ma} >
								<CheckboxCore name={'1'}/>
								<CheckboxCore name={'2'}/>
								<CheckboxCore name={'3'}/>
								<CheckboxCore name={'4'}/>
...
describe("Test CheckboxGroup component - Min/Max tests", () => {
	...
	it("Checkbox group - 5 checkboxes, max=4, min=2, 3 checked", function() { _verifyMinMax(2, 4, 3, 'Limit exceed!!', false); });
	it("Checkbox group - 5 checkboxes, max=4, min=2, 1 checked", function() { _verifyMinMax(2, 4, 4, 'Limit exceed!!', false); });
	it("Checkbox group - 5 checkboxes, max=4, min=2, 5 checked", function() { _verifyMinMax(2, 4, 5, 'Limit exceed!!', true); });
	it("Checkbox group - 5 checkboxes, max=4, min=2, 1 checked no Limmit message", function() { _verifyMinMax(2, 4, 1, '', true); });
	it("Checkbox group - 5 checkboxes, max=4, min=2, 5 checked no Limmit message", function() { _verifyMinMax(2, 4, 5, '', true); });

	it("Checkbox group - 5 checkboxes, max=4, min=2, 3 checked", function() {
		const component = mount(<CheckboxGroupCore title={'CheckboxGroup test'} max={4} min={2} limitMessage={'Limit exceed!!'} >
									<CheckboxCore name={'1'}/>
		                            <CheckboxCore name={'2'}/>
									<CheckboxCore name={'3'}/>
									<CheckboxCore name={'4'}/>
