...
export class Dropdown extends React.PureComponent {
   ...
   */
  setOptions(props, autocomplete, propChange, viaIndex) {
    let returnDisplay = "";
    let returnValue = "";
    ...
    const checkValue = (value, text, i) => {
      const setValue =
        typeof this.state !== "undefined" && !propChange
          ? this.state.value
          : props.value;
      ...
      if (
        ...
        viaIndex && i === this.state.selectedIndex
      ) {
        returnDisplay = text;
        selectedIndex = i;
        returnValue = value;
    ...
    const getOptions = startObject => {
      ...
      if (viaIndex) {
        ...
        optionsArray = matchSorter(startObject, this.state.display, {
          "keys": ["text"]
        });
      }
      return optionsArray;
    ...
    if (props.options) {
      ...
      options = getOptions(props.options).map((option, i) => 
        <Option
          hover={
            typeof this.state !== "undefined" && i === this.state.tempIndex
          }
          index={i}
          key={i}
          onClick={this.optionOnClick}
          onMouseOver={this.optionOnMouseOver}
      ...
      options = getOptions(childArray).map((child, i) => {
        return cloneElement(props.children[child.index], {
          ...
          "index": i,
          "key": i,
          "optionOnClick": this.optionOnClick,
          "optionOnMouseOver": this.optionOnMouseOver,
  ...
  optionOnClick = (e, value, index) => {
    if (value !== this.state.value) {
      ...
      this.setState(
        { "tempIndex": null, "selectedIndex": index, "value": value },
        () => {
          const data = this.setOptions(this.props);
          this.setState(
            {
              "options": data.options,
              "display": data.display
  ...
  optionOnMouseOver = (e, index) => {
    this.setState(
      ...
      () => {
        const data = this.setOptions(this.props, false, false, true);
        this.setState({
          "options": data.options
        });
      }
  ...
  _handleChange = (e, returnData) => {
    ...
    if (this.props.autocomplete) {
      ...
      this.setState(
        ...
        () => {
          const data = this.setOptions(this.props, true);
          this.setState(
            {
              "options": data.options,
              "value": data.value
